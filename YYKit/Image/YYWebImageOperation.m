...
+ (void)_networkThreadMain:(id)object {
   @autoreleasepool {
      [[NSThread currentThread] setName:@"com.idea.foundation.webimage.request"];
   ...
   dispatch_once(&onceToken, ^{
      ...
      if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
         for (NSUInteger i = 0; i < queueCount; i++) {
            ...
            queues[i] = dispatch_queue_create("com.idea.foundation.decode", attr);
         }
      } else {
         for (NSUInteger i = 0; i < queueCount; i++) {
            queues[i] = dispatch_queue_create("com.idea.foundation.decode", DISPATCH_QUEUE_SERIAL);
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
   @autoreleasepool {
      ...
      if (![self isCancelled]) {
         ...
         NSError *error = nil;
         if (!image) {
            error = [NSError errorWithDomain:@"com.idea.foundation.image" code:-1 userInfo:@{ NSLocalizedDescriptionKey : @"Web image decode fail." }];
